...
export function decodeJsonSegment(segment, what) {
    let parsed;
    try {
 ...
 */
export function createNrasTokenVerifier(options = {}) {
    const { jwksUrl = NRAS_JWKS_URL, cacheTtlMs = DEFAULT_CACHE_TTL_MS, clockSkewSec = DEFAULT_CLOCK_SKEW_SEC, pinnedCertSha256 = [], fetchImpl, now = () => Date.now(), } = options;
    ...
    async function importVerifyKey(key) {
        ...
        return crypto.subtle.importKey('jwk', { kty: 'EC', crv: 'P-384', x: key.x, y: key.y, ext: true }, { name: 'ECDSA', namedCurve: 'P-384' }, false, ['verify']);
    }
    /**
     ...
     * parser to get wrong.
     */
    function leafCarriesKey(leafDer, key) {
    ...
    return async function verifyNrasToken(token) {
        ...
        const valid = await crypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-384' }, cryptoKey, toBuffer(signatureBytes), toBuffer(signingInput));
        if (!valid) {
...
//# sourceMappingURL=nras-jwks.js.map